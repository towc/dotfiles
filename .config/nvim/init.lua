...
keymap(
	"nv",
	"<leader>oo",
	":cd /home/user/Documents/obsidian-private/main<cr>:e main.md<cr>",
	{ desc = "open obsidian" }
)
-- }}}
-- run (\r) {{{
...
require("lazy").setup({
	...
	{ -- Fuzzy Finder (files, lsp, etc)
		...
		config = function()
			...
			vim.keymap.set(
				"n",
				"<leader><leader>s",
				defaulted(builtin.builtin),
				{ desc = "[/]earch [S]elect Telescope" }
			)
			vim.keymap.set(
				"n",
				"<leader><leader>w",
				defaulted(builtin.grep_string),
				{ desc = "[/]earch current [W]ord" }
			)
			vim.keymap.set("n", "<leader><leader>g", defaulted(builtin.live_grep), { desc = "[/]earch by [G]rep" })
			vim.keymap.set(
				"n",
				"<leader><leader>d",
				defaulted(builtin.diagnostics),
				{ desc = "[/]earch [D]iagnostics" }
			)
			vim.keymap.set("n", "<leader><leader>r", defaulted(builtin.resume), { desc = "[/]earch [R]esume" })
			vim.keymap.set(
				"n",
				"<leader><leader>.",
				defaulted(builtin.oldfiles),
				{ desc = '[/]earch Recent Files ("." for repeat)' }
			)
		end,
	},
	...
	},
	{
		...
		config = function()
			...
			-- workaround to override the OpenCode permissions (set globally)
			...
			vim.api.nvim_create_user_command("Ai", function(data)
